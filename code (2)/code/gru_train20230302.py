...
for train_name in train_list:
    centroid_csv = os.path.join(root_dir, train_name, 'df_ankle_ofs.csv')
...
for valid_name in valid_list:
    centroid_csv = os.path.join(root_dir, valid_name, 'df_ankle_ofs.csv')
    centroids = pd.read_csv(centroid_csv).values
...
for epoch in range(epochs):
    ...
    for index in index_list:
        ...
        if train_step % 100 == 0:
            ...
            train_df.to_csv(os.path.join(start_time, 'train_loss.csv'), index=False,mode='a')
            record_train_loss.clear()
    ...
    with torch.no_grad():
        for index in range(len(valid_dataset)):
            ...
            if valid_step % 10 == 0:
                ...
                valid_df.to_csv(os.path.join(start_time, 'valid_loss.csv'), index=False)
                record_valid_loss.clear()
...
valid_df = pd.DataFrame(record_valid_loss)
train_df.to_csv(os.path.join(start_time, 'train_loss.csv'), index=False,mode='a')
valid_df.to_csv(os.path.join(start_time, 'valid_loss.csv'), index=False,mode='a')
writer.close()
