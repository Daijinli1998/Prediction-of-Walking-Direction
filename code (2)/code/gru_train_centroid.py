...
train_dataset_x = train_dataset[:, :, :, 0]
train_dataset_z = train_dataset[:, :, :, 1]
train_label_x = train_label[:, :, 0]
train_label_z = train_label[:, :, 1]
...
for epoch in range(epochs):
    ...
    for index in index_list:
        data_tensor = torch.from_numpy(train_dataset_x[index]).cuda()
        label_tensor = torch.from_numpy(train_label_x[index]).cuda()
    ...
    record_valid_loss.append(valid_loss_sum)
    record_train_loss.append(train_loss_sum)
...
train_df.to_csv("D:\\PyCharmProject\\my_project\\gru_train_loss.csv")
valid_df = pd.DataFrame(record_valid_loss)
valid_df.to_csv("D:\\PyCharmProject\\my_project\\gru_valid_loss.csv")
