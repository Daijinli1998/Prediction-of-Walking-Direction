...
from util import RgbDataset
import pandas as pd

normal_img_dir = 'D:\\我的\\研途\\毕业论文研\\我\\数据\\all_in_one\\all\\normal\\img'
abnormal_img_dir = 'D:\\我的\\研途\\毕业论文研\\我\\数据\\all_in_one\\all\\abnormal\\img'

normal_csv_cnt = len(os.listdir(normal_img_dir))
abnormal_csv_cnt = len(os.listdir(abnormal_img_dir))
...
train_dataset = RgbDataset(normal_img_dir, train_normal_index_list[:2000], 0,
                           trans=transforms.Compose([transforms.ToTensor(), transforms.Resize((32, 32))]))

train_dataset += RgbDataset(abnormal_img_dir, train_abnormal_index_list[:500], 1,
                            trans=transforms.Compose([transforms.ToTensor(), transforms.Resize((32, 32))]))
...
train_dataset += RgbDataset(abnormal_img_dir, train_abnormal_index_list[:500], 1, img_size=50,
                            trans=transforms.Compose([transforms.ToTensor(), transforms.CenterCrop((32, 32))]))

valid_dataset = RgbDataset(normal_img_dir, valid_normal_index_list[:600], 0,
                           trans=transforms.Compose([transforms.ToTensor(), transforms.Resize((32, 32))]))

valid_dataset += RgbDataset(abnormal_img_dir, valid_abnormal_index_list[:150], 1,
                            trans=transforms.Compose([transforms.ToTensor(), transforms.Resize((32, 32))]))
valid_dataset += RgbDataset(abnormal_img_dir, valid_abnormal_index_list[:150], 1,
                            trans=transforms.Compose([transforms.ToTensor(), transforms.Resize((32, 32)),
                                                      transforms.RandomRotation(15)]))
valid_dataset += RgbDataset(abnormal_img_dir, valid_abnormal_index_list[:150], 1,
                            trans=transforms.Compose([transforms.ToTensor(), transforms.Resize((32, 32)),
                                                      transforms.RandomHorizontalFlip(1.0)]))
valid_dataset += RgbDataset(abnormal_img_dir, valid_abnormal_index_list[:150], 1, img_size=50,
                            trans=transforms.Compose([transforms.ToTensor(), transforms.CenterCrop((32, 32))]))
